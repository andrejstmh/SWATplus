...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      use hru_module, only : hru, surqsolp,  surfq, i_sep, ihru, qtile 
      use soil_module
...
!! compute soluble P leaching
      vap = soil1(j)%mp(1)%lab * soil(j)%ly(1)%prk /                   &
        ((soil(j)%phys(1)%conv_wt/ 1000.) * bsn_prm%pperco + .1)   !dont merge
      vap = Min(vap, .5 * soil1(j)%mp(1)%lab)
      call debugprint(1, 'solp_percnlyr-', -vap)
      soil1(j)%mp(1)%lab = soil1(j)%mp(1)%lab - vap
      if (soil(j)%nly >= 2) then
          call debugprint(2, 'solp_percnlyr+', vap)
      ...
      do ii = 2, soil(j)%nly-1
	   ...
	   if (ii/=i_sep(j)) then
	     ...
	     soil1(j)%mp(ii+1)%lab = soil1(j)%mp(ii+1)%lab + vap
          call debugprint(ii, 'solp_percnlyr-', -vap)
          call debugprint(ii+1, 'solp_percnlyr+', vap)
         ...
         if (ii == hru(j)%lumv%ldrain) then
           ...
           soil1(j)%mp(ii)%lab = soil1(j)%mp(ii)%lab - vap
           call debugprint(ii, 'solp_tile', -vap)
         endif
	   endif
	 end do
	 !percp(j) = vap
